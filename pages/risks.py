...
controls = html.Div(id="rk-controls", children=[
    html.Div(className="control", children=[html.Label("Tickers", htmlFor="rk-tickers"),
        dcc.Checklist(id="rk-tickers", value=TICKERS_DEFAULT,
                      options=[{"label": t, "value": t} for t in TICKERS_DEFAULT], inline=True)]),
    html.Div(className="control", children=[html.Label("Rolling Window (trading days)", htmlFor="rk-window"),
        dcc.Slider(id="rk-window", min=10, max=120, step=5, value=30, marks={10:"10",30:"30",60:"60",120:"120"})]),
    html.Div(className="control", children=[html.Label("Date Range", htmlFor="rk-dates"),
        dcc.DatePickerRange(id="rk-dates",
            min_date_allowed=PRICES["date"].min(),
            max_date_allowed=PRICES["date"].max(),
            start_date=PRICES["date"].min(), end_date=PRICES["date"].max(),
...
_page = html.Div(id="rk-page", children=[
    ...
    html.Section(id="rk-body", children=[
        ...
        html.Section(id="rk-content", children=[dcc.Graph(id="rk-chart", config={"displayModeBar": False})]),
    ]),
])
